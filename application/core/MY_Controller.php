...
class MY_Controller extends CI_Controller{
	...
	function ajax_panel($name, $params = array()){
		...
		// if no cms_page_panel_id , get largest with page id 999999
		if (empty($params['cms_page_panel_id'])){
			$params['cms_page_panel_id'] = $this->cms_page_panel_model->get_max_cms_page_panel_id($name);
		}

		// if cms_page_panel_id, load panel params from db
		if (!empty($params['cms_page_panel_id'])) {
			$params = array_merge($this->cms_page_panel_model->get_cms_page_panel($params['cms_page_panel_id']), $params);
		}
